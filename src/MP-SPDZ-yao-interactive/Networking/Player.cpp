...
{
  //#ifdef VERBOSE_COMM
  cout << "sending to " << player << endl;
  //#endif
  TimeScope ts(comm_stats["Sending directly"].add(o));
  send_to_no_stats(player, o);
  sent += o.get_length();
  cout << "sent " << o.get_length() << endl;
}

void Player::send_to_and_writefile(int player,const octetStream& o, string fname) const
{
  ...
  if (!in.good()) {
    ...
    cout << "GC file already exists, skipping" << endl;
  }
  ///////////////////

  cout << "sending to " << player << " and writing " << fname << endl;

  octetStream z = octetStream(0);
  
  TimeScope ts(comm_stats["Sending directly"].add(z));
  send_to_no_stats(player, z);
  sent += z.get_length();
  cout << "sent " << z.get_length() << endl;
}
...
void Player::receive_player_and_readfile(int i,octetStream& o, string fname) const
{
  DoubleTimer ts2;
  ts2.start();  
  ...
  in.close();
  ts2.stop();
  cout << "READING ELAPSED: " << ts2.elapsed() << endl;
  ///////////////
}
...
void Player::receive_player_and_readfile(int i, FlexBuffer& buffer, string fname) const
{
  octetStream os;
  receive_player_and_readfile(i, os, fname);
  buffer = os;
}
