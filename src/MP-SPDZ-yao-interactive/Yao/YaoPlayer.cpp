...
YaoPlayer::YaoPlayer(int argc, const char** argv)
{
	...
	opt.add(
			...
			"gc saveprefix", // Help description.
			"-gcs", // Flag token.
			"--gcs_saveprefix" // Flag token.
			);
	...
	string gcs_saveprefix;
	opt.get("-gcs")->getString(gcs_saveprefix);

	online_opts.progname = progname;
	online_opts.playerno = my_num;
	online_opts.interactive = 1;
	online_opts.gcs_saveprefix = gcs_saveprefix;
